...
export const executeStatement = async (
  ...
  try {
    ...
    const queryStats = {
      consumedIOs: result.reduce(
        (acc, res) => [...acc, res.getConsumedIOs()],
        []
      ),
      timingInformation: result.reduce(
        (acc, res) => [...acc, res.getTimingInformation()],
        []
      ),
    };
    recordHistory(
      statement,
      result.reduce((acc, res) => [...acc, ...res.getResultList()], []),
      queryStats,
      setHistory
    );
    return {
