      ...
      <Grid container spacing={0}>
        <Grid item xs={12}>
          <Typography variant="caption">
            {" "}
            Read IOs: {queryStats.consumedIOs.readIOs}
          </Typography>
        </Grid>
        <Grid item xs={12}>
          <Typography variant="caption">
            Time: {queryStats.timingInformation.processingTimeMilliseconds}ms
          </Typography>
        </Grid>
      </Grid>
    )
...
const prepareErrorView = (error: string) =>
  error && (
      ...
      <Table className={classes.table} size="small" aria-label="Results table">
          ...
          <TableRow>
            {headers.map((header) => (
              <TableCell key={header}>
                <Typography className={classes.tableHeadContent}>
                  {header}
                </Typography>
              </TableCell>
            ))}
        ...
        <TableBody>
          {queryResult.map((result) =>
            prepareTableContentRow(result, headers, rowIndex++)
          )}
        </TableBody>
      </Table>
    </TableContainer>
...
function prepareTableContentRow(
  queryResult: {},
  headers: [],
  rowIndex: number
) {
    ...
    <TableRow key={rowIndex}>
      {headers.map((header) => (
        <TableCell key={`cell${cellIndex++}`}>
          {JSON.stringify(queryResult[header])}
        </TableCell>
      ))}
    </TableRow>
  );
