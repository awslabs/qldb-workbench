...
function handleWidthDragging(state, action) {
  switch (action.type) {
    ...
    case "mousemove":
      ...
      return {
        ...state,
        currentX: action.clientX,
        width: width < MIN_TOOL_WINDOW_WIDTH ? MIN_TOOL_WINDOW_WIDTH : width,
      };
    default:
      return state;
  }
...
function handleHeightDragging(state, action) {
  switch (action.type) {
    ...
    case "mousemove":
      ...
      return {
        ...state,
        currentY: action.clientY,
        height:
          height < MIN_TOOL_WINDOW_HEIGHT ? MIN_TOOL_WINDOW_HEIGHT : height,
      };
    default:
      return state;
  }
...
function useWidthDraggableHandle(id: string, invert: boolean) {
  const [handleState, dispatch] = useReducer(handleWidthDragging, {
    id,
    invert,
    dragging: false,
  });
...
function useHeightDraggableHandle(id: string, invert: boolean) {
  const [handleState, dispatch] = useReducer(handleHeightDragging, {
    id,
    invert,
    dragging: false,
  });
...
function BrowseSidebar({ navEl, width, dispatchLeft }) {
  ...
  return open ? (
    <>
      <div
        id="righthandle"
        className="handle"
        onMouseDown={dispatchRight}
        onMouseMove={dispatchRight}
      />
      <div ref={toolsEl} id="tools" style={{ width: width + "px" }}>
        <Tool name="History" close={toggleOpen}>
    ...
    <>
      <aside className="right collapsed">
        <ul>
          <li className="right" onClick={toggleOpen}>
            <TextIcon name="history" />
            <span className="right">History</span>
          </li>
        </ul>
      </aside>
  ...
  return (
    <section className="editors">
      <ul className="buffers">
        <li>
          <Button name="Buffer One" />
        </li>
        <li>
          <Button name="Buffer Two" />
        </li>
        <li>
          <Button name="Buffer Three" />
        </li>
      </ul>
...
function Workbench() {
  const [leftHandleState, navEl, dispatchLeft] = useWidthDraggableHandle(
    "left",
    false
  );
  const [rightHandleState, toolsEl, dispatchRight] = useWidthDraggableHandle(
    "right",
    true
  );
  const [bottomHandleState, resultsEl, dispatchBottom] =
    useHeightDraggableHandle("bottom", true);
  const dispatchAll = (e) => {
    dispatchLeft(e);
    dispatchRight(e);
    dispatchBottom(e);
  ...
  return (
    <ThemeProvider className="root">
      ...
      <main
        style={{
          userSelect:
            leftHandleState.dragging ||
            rightHandleState.dragging ||
            bottomHandleState.dragging
              ? "none"
              : "auto",
        }}
      ...
      >
        <BrowseSidebar
          {...{ navEl, width: leftHandleState.width, dispatchLeft }}
        />
        <Editors
          {...{ resultsEl, height: bottomHandleState.height, dispatchBottom }}
        />
