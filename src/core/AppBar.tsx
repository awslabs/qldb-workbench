...
export default ({region, setRegion, darkState, setDarkState, showInactive, setShowInactive, setForceRefresh}:
                    { region: string, setRegion: (region: string) => void, darkState: boolean, setDarkState: (state: boolean) => void, showInactive: boolean, setShowInactive: (state: boolean) => void, setForceRefresh: (refresh: boolean) => void }) => {
        ...
        return (
            <FormControl className={classes.formControl}>
                <Select
                    ...
                    id="Region"
                    value={region}
                    onChange={(event: React.ChangeEvent<{ value: unknown }>) => setRegion(event.target.value as string)}
                    className={classes.select}
                    inputProps={{classes: {icon: classes.icon}}}
                >
                    {qldbRegions.map(region => <MenuItem key={region.region}
                                                         value={region.region}>{region.name}</MenuItem>)}
                </Select>
            </FormControl>
        )
    }
    ...
    const MenuActions = () => {
        const showInactiveIcon = showInactive ? <VisibilityOffIcon/> : <VisibilityIcon/>;
        ...
        return (
            <Menu id="menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleMenuClose}>
                <MenuItem onClick={handleMenuClose}>
                    <Button startIcon={showInactiveIcon} onClick={toggleShowInactive} className={classes.menuItem}
                            disableElevation={true}>
                        {showInactive ? "Hide inactive tables" : "Show inactive tables"}
                    </Button>
                </MenuItem>
                <MenuItem onClick={handleMenuClose}>
                    <Button startIcon={preferenceIcon} className={classes.menuItem} onClick={handleDialogOpen}
        ...
        return (
            <Dialog fullScreen={fullScreen} open={dialogOpen} onClose={handleMenuClose}
                ...
                <DialogContent>
                    <Accordion>
                        <AccordionSummary
                            ...
                            <div className={classes.twoThirdColumn}>
                                <Typography className={classes.heading}>AWS account credentials</Typography>
                            </div>
                        </AccordionSummary>
                        <AccordionDetails>
                            <div className={classes.twoThirdColumn}>
                                <FormControl fullWidth>
                                    <TextField autoFocus margin="dense" id="awsAccessKey" label="AWS Access Key Id"
                                               type="text" fullWidth style={{paddingRight: "15px"}}
                                               inputRef={awsAccessKey}/>
                                    <TextField margin="dense" id="awsSecretKey" label="AWS Secret Access Key"
                                               type="password"
                                               fullWidth style={{paddingRight: "15px"}}
                                               inputRef={awsSecretKey}/>
                                    <TextField margin="dense" id="awsSessionToken" label="AWS Session Token"
                                               type="password"
                                               fullWidth style={{paddingRight: "15px"}}
                    ...
                    <Accordion>
                        <AccordionSummary
                            expandIcon={<ExpandMoreIcon/>} aria-controls="panel1c-content" id="panel1c-header">
                            <div className={classes.twoThirdColumn}>
                                <Typography className={classes.heading}>Endpoint override</Typography>
                            </div>
                        </AccordionSummary>
                        <AccordionDetails>
                            <div className={classes.twoThirdColumn}>
                                <FormControl fullWidth>
                                    <TextField autoFocus margin="dense" id="frontendEndpoint" label="Frontend Endpoint"
                                               type="url" fullWidth style={{paddingRight: "15px"}} name="frontendEndpoint"
                                               inputRef={frontendEndpoint} helperText={frontendEndpointValue ? `Current value: ${frontendEndpointValue}` : ``}/>
                                    <TextField margin="dense" id="sessionEndpoint" label="Session Endpoint" type="url"
                                               fullWidth style={{paddingRight: "15px"}} name="sessionEndpoint"
                                               inputRef={sessionEndpoint} helperText={sessionEndpointValue ? `Current value: ${sessionEndpointValue}` : ``}/>
                                </FormControl>
                            </div>
                            <div className={classes.helper}>
                                <Typography variant="caption">
                                    Overrides endpoints to be used by qldb frontend and session API.
                                </Typography>
                            </div>
                        </AccordionDetails>
                    </Accordion>
                ...
                <DialogActions>
                    ...
                    <Button size="small" color="primary" onClick={handleConfigUpdate}>Update</Button>
                </DialogActions>
            </Dialog>
        )
    }
    ...
    return (
        <AppBar position="fixed" className={classes.appBar}>
            <Toolbar>
                ...
                <IconButton aria-label={"Menu"} component={"span"} onClick={handleMenuClick}><MoreVert
