...
export type QueryStats = {
  timingInformation: { processingTimeMilliseconds: number };
  consumedIOs: { readIOs: number };
};
...
export type SetHistoryFn = (
  value:
    | ((prevState: QueryHistoryEntry[]) => QueryHistoryEntry[])
    | QueryHistoryEntry[]
) => void;

export function loadHistory(setHistory: SetHistoryFn) {
  readline
    .createInterface(fs.createReadStream(HISTORY_FILE))
    .on("line", (l) => {
