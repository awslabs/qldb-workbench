...
const App = () => {
  ...
  const setActiveLedger = (name: string) => {
    setActiveLedgerState(name);
    activeLedger.current = name;
  };
  ...
  const [queryStats, setQueryStats] = React.useState(
    undefined as unknown as QueryStats
  );
  ...
  AWS.config.logger = {
    log: (...messages: any[]) => {
      ...
      console.log(data);
      fs.appendFileSync(".qldb-workbench.log", data);
    },
  };
            ...
            >
              ...
              <div className={classes.drawerContainer}>
                <DrawerItems
                  ...
                  setActiveLedger={setActiveLedger}
                  showInactive={showInactive}
                  forceRefresh={forceRefresh}
                  setForceRefresh={setForceRefresh}
                  activeLedger={activeLedgerState}
                />
              </div>
            </Drawer>
            <main className={classes.content}>
              ...
              <Paper>
                <SplitPane
                  split="horizontal"
                  size="50%"
                  style={{ paddingTop: "73px", maxWidth: splitPaneWidth }}
                  pane2Style={{ overflow: "scroll" }}
                >
                  <Composer
                    executeStatement={() =>
                      execute(
                        activeLedger.current,
                        setHistory,
                        setResult,
                        setError,
                        setQueryStats
