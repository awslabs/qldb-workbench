...
const GoSvg = ({ color }: { color: Color }) =>
    <svg width="20px" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="running"
         ...
         className="svg-inline--fa fa-running fa-w-13 fa-2x"><path fill={color}
...
const ActionBar = () => {
    return <span className="action-bar">
        <span className="action-button"><GoSvg color={Color.GREEN} /></span>
    </span>;
}
...
const Composer = () => {
    ...
    return <div style={{ width: "100%", display: "flex", flexDirection: "column" }}>
        <ContentEditable html={text.current} onChange={handleChange} style={EDITOR_STYLE} />
        <ActionBar />
    </div>
}
...
const Results = () => {
    ...
    return <ContentEditable html={text.current} onChange={handleChange} style={Object.assign({ backgroundColor: Color.LIGHTGRAY, flex: 1 }, EDITOR_STYLE)} />;
}
